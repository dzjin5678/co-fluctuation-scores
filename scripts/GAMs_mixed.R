...
gam.fit.smooth <- function(region, smooth_var, covariates, knots, set_fx = FALSE, stats_only = FALSE){
  ...
  parcel <- region
  #Fit the gam
  modelformula <- as.formula(sprintf("%s ~ s(%s, k=%s, fx=%s) + s(subj_id, bs='re') + %s", region, smooth_var, knots, set_fx, covariates))
  ...
  full.results <- cbind(parcel, gam.smooth.F, gam.smooth.pvalue, partialRsq, anova.smooth.pvalue, 
                        change.onset, peak.change, decrease.onset, increase.offset, change.offset, mean.derivative_2l)
