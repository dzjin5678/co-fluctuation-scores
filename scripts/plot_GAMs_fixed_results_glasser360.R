...
glasser360_label <- read.csv("../../atlas/glasser360_roi_names.csv", header = F)
glasser360_roi_names <- glasser360_label$V1
glasser360_roi_names[1:180]  <- paste0("rh_", glasser360_roi_names[1:180])
glasser360_roi_names[181:360] <- paste0("lh_", glasser360_roi_names[181:360])
glasser360_roi_names <- sub("_ROI$", "", glasser360_roi_names)

############################## load plot data  #################################
# GAM fit
gam.amp.glasser360 <- read.csv(gam_amp_filename)
# GAM smooth estimates
gam.smoothestimates.glasser360 <- read.csv(gam_estimated_smooths_filename)
gam.smoothestimates.glasser360 <- merge(gam.smoothestimates.glasser360, gam.amp.glasser360, by="label", sort = F)
# GAM smooth.
gam.smooths.glasser360 <- read.csv(gam_smooths_filename)
gam.smooths.glasser360 <- merge(gam.smooths.glasser360, gam.amp.glasser360, by="label", sort = F)
# GAM data
...
min_limit <- min(gam.smoothestimates.glasser360$est);
max_limit <- max(gam.smoothestimates.glasser360$est);

# display all trajectories colored by partial R2.
q_r2 <- quantile(gam.smoothestimates.glasser360$GAM.age.partialR2, probs = c(0.25, 0.75), na.rm = FALSE)
df <- gam.smoothestimates.glasser360
df_filtered <- df %>% filter(GAM.age.partialR2 <= q_r2[1] | GAM.age.partialR2 >= q_r2[2])

color_full <- gam.smoothestimates.glasser360$GAM.age.partialR2
...
q_r2 <- quantile(gam.smoothestimates.glasser360$mean.derivative_2l, probs = c(0.25, 0.75), na.rm = FALSE)
df <- gam.smoothestimates.glasser360
df_filtered <- df %>% filter(mean.derivative_2l <= q_r2[1] | mean.derivative_2l >= q_r2[2])

color_full <- gam.smoothestimates.glasser360$mean.derivative_2l
...
################################ amplitude effect ##############################
effect_data <- gam.amp.glasser360
some_data = tibble(
  label = glasser360_roi_names,
...
for (lr in c("left", "right")){
  for (lm in c("lateral", "medial")){
    ...
    p <- ggseg(.data = some_data, atlas = glasser, mapping=aes(fill=p, colour=I("white")), position = c("stacked"), hemisphere=lr, view=lm) +
      theme_void() +
      labs(fill="") +
...
p <- ggseg(.data = some_data, atlas = glasser, mapping=aes(fill=p, colour=I("white")), position = c("stacked")) +
  theme_void() +
  labs(fill="") +
...
############################# second derivations ###############################
second_derivatives_data <- gam.amp.glasser360
some_data = tibble(
  label = glasser360_roi_names,
