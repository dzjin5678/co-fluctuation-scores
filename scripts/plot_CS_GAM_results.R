...
pipeline <- "icafix_ts" # icafix_ts icafixgs_ts
smooth_var <- "zscored_bins_rss"
covariates <- "sex_label + mean_fd + mean_gs + mean_br + mean_hr"

save_dir <- sprintf("../results_bins/DRIVER/%s/region_level_%s/%s", dataset, cs_name, pipeline)
dir.create(save_dir)

schaefer200x17_atlas <- read.csv(sprintf("../../atlas/%s_atlas.CSV", atlas))
...
############################## load plot data  #################################
# GAM fit
gam.amp.schaefer200x17 <- read.csv(
...
gam.smoothestimates.schaefer200x17 <- read.csv(
...
# GAM smooth.
gam.smooths.schaefer200x17 <- read.csv(
...
colnames(gam.data)[202] <- "sex_label"
...
#   file_path_name = sprintf("../../../results_bins/DRIVER/%s/region_level/ALL_SESSION_%s/%s_%s_%s_%s.jpeg", dataset, pipeline, session, atlas, pipeline, region)
...
#   region <- sprintf("V%s", row)
#   V1.smooth <- gam.smooths.schaefer200x17 %>% filter(label == region)
#   file_path_name = sprintf("../../../results_bins/DRIVER/%s/region_level/ALL_SESSION_%s/%s_%s_%s_%s.emf", dataset, pipeline, session, atlas, pipeline, region)
#   gam_data <- gam.data %>% select(region, raw_rss)
#   colnames(gam_data)[1] <- "region_roi"
#   scatterplot <- ggplot(data=gam_data, aes(x = raw_rss, y = region_roi)) +
...
for(subnet_id in c(1:8)){
  ...
  file_path_name = sprintf("../results_bins/DRIVER/%s/region_level_%s/%s/%s.jpeg", dataset, cs_name, pipeline, as.character(subnet_id))
  jpeg(file = file_path_name,
...
for(subnet_id in c(1:8)){
  subnet_data <- gam.smoothestimates.schaefer200x17[gam.smoothestimates.schaefer200x17$yeo8_label == subnet_id, ]
  file_path_name = sprintf("../results_bins/DRIVER/%s/region_level_%s/%s/%s.emf", dataset, cs_name, pipeline, as.character(subnet_id))
  a <- ggplot(subnet_data, aes(zscored_bins_rss, est, group=index, color=color_arr[subnet_id])) +
...
file_path_name = sprintf("../results_bins/DRIVER/%s/region_level_%s/%s/all_primary_effect.jpeg", dataset, cs_name, pipeline)
...
color_full <- gam.smoothestimates.schaefer200x17$GAM.age.partialR2
color <- df_filtered$GAM.age.partialR2
file_path_name = sprintf("../results_bins/DRIVER/%s/region_level_%s/%s/all_primary_effect.emf", dataset, cs_name, pipeline)
...
file_path_name = sprintf("../results_bins/DRIVER/%s/region_level_%s/%s/all_second_effect.jpeg", dataset, cs_name, pipeline)
jpeg(file = file_path_name, width=1600, height=1500, units = "px",res =105)
ggplot(df_filtered, aes(zscored_bins_rss, est, group=index, color=color)) +
...
color_full <- gam.smoothestimates.schaefer200x17$mean.derivative_2l
color <- df_filtered$mean.derivative_2l
file_path_name = sprintf("../results_bins/DRIVER/%s/region_level_%s/%s/all_second_effect.emf", dataset, cs_name, pipeline)
p <- ggplot(df_filtered, aes(zscored_bins_rss, est, group=index, color=color)) +
  geom_line(size=0.5, alpha = 0.8) +
  paletteer::scale_color_paletteer_c("grDevices::Geyser", direction = 1, limits = c(min(color), max(color))) +
  theme_classic() +
...
################################ amplitude effect ##############################
# effect_data <- read.csv(sprintf("../../../results_bins/DRIVER/%s/region_level/ALL_SESSION_%s/gam_amp_%s_%s_raw_rss.csv", 
...
for (lr in c("left", "right")){
  for (lm in c("lateral", "medial")){
    file_path_name = sprintf("../results_bins/DRIVER/%s/region_level_%s/%s/all_primary_effect_%s_%s.jpeg", dataset, cs_name, pipeline, lr, lm)
...
# single maps.
file_path_name = sprintf("../results_bins/DRIVER/%s/region_level_%s/%s/all_primary_effect_brain_map.jpeg", dataset, cs_name, pipeline)
...
# max_lim <- max(some_data$p)
# min_lim <- min(some_data$p)
# jpeg(file = sprintf("../../../results_bins/DRIVER/%s/region_level/ALL_SESSION_%s/%s_amplitude_effect_%s_%s_zscored_bins_rss_V45.jpeg",
...
for (lr in c("left", "right")){
  for (lm in c("lateral", "medial")){
    file_path_name = sprintf("../results_bins/DRIVER/%s/region_level_%s/%s/all_second_effect_%s_%s.jpeg", dataset, cs_name, pipeline, lr, lm)
    jpeg(file = file_path_name, width = 1500, height = 1000, units = "px", res = 800)
    p <- ggseg(.data = some_data, atlas = schaefer17_200, mapping=aes(fill=p, colour=I("white")), position = c("stacked"), hemisphere=lr, view=lm) +
      theme_void() +
      labs(fill="") +
...
# four map.
file_path_name = sprintf("../results_bins/DRIVER/%s/region_level_%s/%s/all_second_effect_brain_map.jpeg", dataset, cs_name, pipeline)
jpeg(file = file_path_name, width=1500, height=1500, units = "px",res =300)
p <- ggseg(.data = some_data, atlas = schaefer17_200, mapping=aes(fill=p, colour=I("white")), position = c("stacked")) +
  theme_void() +
  labs(fill="") +
...
# P1
# jpeg(file = sprintf("../../../results_bins/DRIVER/%s/region_level/ALL_SESSION_%s/stat_amplitude_effect_%s_%s.jpg", dataset, pipeline, atlas, pipeline),
...
#   scale_x_discrete(labels = c("VIS", "SMN", "DAN", "SVAN", "LIMB", "CONT", "DMN", "TP"))
# P1
# jpeg(file = sprintf("../../../results_bins/DRIVER/%s/region_level/ALL_SESSION_%s/stat_second_derivatives_%s_%s.jpg", dataset, pipeline, atlas, pipeline),
#      width =1200,height = 1200,units = "px",res =450)
# print(P1)
# dev.off()
...
file_path_name = sprintf("../results_bins/DRIVER/%s/region_level_%s/%s/scatter_partialR2_secondDerivative.jpeg", dataset, cs_name, pipeline)
...
p7 <- scatterplot + geom_hdr_lines(linewidth=0.3, probs = c(0.4), 
                                   method = "kde", xlim=c(-4, 3), ylim=c(-3, 4))
file_path_name = sprintf("../results_bins/DRIVER/%s/region_level_%s/%s/scatter_partialR2_secondDerivative.emf", dataset, cs_name, pipeline)
