...
# begin gam.
gam.amp.schaefer400x17 <- matrix(data=NA, nrow=400, ncol=12) # matrix to save gam.fit output to
gam.amp.cov.t.schaefer400x17 <- matrix(data=NA, nrow=400, ncol=8) # matrix to save gam.fit output to
gam.amp.cov.p.schaefer400x17 <- matrix(data=NA, nrow=400, ncol=8) # matrix to save gam.fit output to
...
for(row in c(1:num_of_rois)){
  ...
  gam.amp.schaefer400x17[row,] <- GAM.RESULTS$results
  gam.amp.cov.t.schaefer400x17[row,] <- GAM.RESULTS$p.t
  gam.amp.cov.p.schaefer400x17[row,] <- GAM.RESULTS$p.pv
}
...
gam.amp.cov.t.schaefer400x17 <- as.data.frame(gam.amp.cov.t.schaefer400x17)
colnames(gam.amp.cov.t.schaefer400x17) <- c("intercept","sex","age", "tSNR","mean_fd","mean_gs","mean_br","mean_hr")
write.csv(gam.amp.cov.t.schaefer400x17,
          sprintf("../results_bins/DRIVER/%s/%s/cov_t_%s", dataset, cs_name, gam_amp_filename),
          row.names = F, quote = F)
gam.amp.cov.p.schaefer400x17 <- as.data.frame(gam.amp.cov.p.schaefer400x17)
colnames(gam.amp.cov.p.schaefer400x17) <- c("intercept","sex","age", "tSNR","mean_fd","mean_gs","mean_br","mean_hr")
write.csv(gam.amp.cov.p.schaefer400x17,
