...
%% loop settings.
loop_settings(0, 20, 1, 1, 1);
loop_settings(0, 20, 1, 1, 0);
loop_settings(0, 20, 1, 0, 1);
loop_settings(0, 20, 1, 0, 0);
...
function cal_and_plot_bin_corr(high_fd, num_of_bins, pipeline, atlas, regress_cofounds, cs_name, save_dir_name)
    ...
    session_names = ['REST1_LR'; 'REST1_RL'; 'REST2_LR'; 'REST2_RL'];
    rest1_lr = load(['../outputs_bins/HCP/tSNR/bins_', num2str(num_of_bins), '_' pipeline '_scan_' session_names(1, :) '_' atlas '_hfd_' num2str(high_fd) '_regressCofounds_' num2str(regress_cofounds) '_tSNR.mat'], cs_name);
    rest1_rl = load(['../outputs_bins/HCP/tSNR/bins_', num2str(num_of_bins), '_' pipeline '_scan_' session_names(2, :) '_' atlas '_hfd_' num2str(high_fd) '_regressCofounds_' num2str(regress_cofounds) '_tSNR.mat'], cs_name);
    rest2_lr = load(['../outputs_bins/HCP/tSNR/bins_', num2str(num_of_bins), '_' pipeline '_scan_' session_names(3, :) '_' atlas '_hfd_' num2str(high_fd) '_regressCofounds_' num2str(regress_cofounds) '_tSNR.mat'], cs_name);
    rest2_rl = load(['../outputs_bins/HCP/tSNR/bins_', num2str(num_of_bins), '_' pipeline '_scan_' session_names(4, :) '_' atlas '_hfd_' num2str(high_fd) '_regressCofounds_' num2str(regress_cofounds) '_tSNR.mat'], cs_name);
    ...
    save_filename=strcat("bins_corr_high_fd-",num2str(high_fd), ...
        ...
        "_gsr-", pipeline);
    save_path=strcat("../results_bins/tSNR/",save_dir_name);
    mkdir(save_path);
...
function cal_and_plot_cs_sa_association(high_fd, num_of_bins, pipeline, atlas, regress_cofounds, cs_name, save_dir_name)
    ...
    load("../results_bins/tSNR/hcp3t_unrelated100_tSNR_schaefer200.mat");
    brain_map_sa_axis(low_tSNR_roi_idx)=[];
    ...
    save(strcat("../results_bins/tSNR/",save_dir_name, "/", file_name), "roi_contri_group", "coefs");
    % plot results.
    coefs_abs = abs(coefs);
...
function prepare_all_ses_data_for_gam(high_fd, num_of_bins, pipeline, atlas, regress_cofounds, cs_name, save_dir_name, phenotype_data, subj_ids, num_of_rois)
    ...
    session_names = ['REST1_LR'; 'REST1_RL'; 'REST2_LR'; 'REST2_RL'];
    rest1_lr = load(['../outputs_bins/HCP/tSNR/bins_', num2str(num_of_bins), '_' pipeline '_scan_' session_names(1, :) '_' atlas '_hfd_' num2str(high_fd) '_regressCofounds_' num2str(regress_cofounds) '_tSNR.mat'], ...
        cs_name, 'ets_global_rss_bins_all', 'fd_bins_all', 'gs_bins_all', 'br_bins_all', 'hr_bins_all');
    rest1_rl = load(['../outputs_bins/HCP/tSNR/bins_', num2str(num_of_bins), '_' pipeline '_scan_' session_names(2, :) '_' atlas '_hfd_' num2str(high_fd) '_regressCofounds_' num2str(regress_cofounds) '_tSNR.mat'], ...
        cs_name, 'ets_global_rss_bins_all', 'fd_bins_all', 'gs_bins_all', 'br_bins_all', 'hr_bins_all');
    rest2_lr = load(['../outputs_bins/HCP/tSNR/bins_', num2str(num_of_bins), '_' pipeline '_scan_' session_names(3, :) '_' atlas '_hfd_' num2str(high_fd) '_regressCofounds_' num2str(regress_cofounds) '_tSNR.mat'], ...
        cs_name, 'ets_global_rss_bins_all', 'fd_bins_all', 'gs_bins_all', 'br_bins_all', 'hr_bins_all');
    rest2_rl = load(['../outputs_bins/HCP/tSNR/bins_', num2str(num_of_bins), '_' pipeline '_scan_' session_names(4, :) '_' atlas '_hfd_' num2str(high_fd) '_regressCofounds_' num2str(regress_cofounds) '_tSNR.mat'], ...
        cs_name, 'ets_global_rss_bins_all', 'fd_bins_all', 'gs_bins_all', 'br_bins_all', 'hr_bins_all');
    % load phsio data.
    load("../outputs_bins/HCP/physio_phase/BOLD_PLVs_proc_regress_fix_ts_mat_schaefer200.mat", "physios_avg1", "physios_ds");
    HR_all = permute(physios_avg1, [2 3 1]);
